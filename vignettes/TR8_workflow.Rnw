...
\title{A 'real life' workflow for the \texttt{TR8} package}
...
\item[env\_data] contains the level for the experimental factors
  \end{description}
...
\end{verbatim}
  ...
  (i.e. those entries for which the score is "0.9") from the original \texttt{metadata} dataframe. 
     ...
     codes used by authors)  and the corrected ones:
@ 
<<two,eval=FALSE>>=
...
@ %def 
  ...
  Then we exlude those species which we identified at the Genus level, i.e. those contained in the \textit{issues} dataframe:
...
\end{enumerate}
 ...
 \begin{itemize}
 \item for for \textit{C. arvensis} we calculate the mean between 75 and 10 and use that as the \texttt{h\_max}
@ 
<<cirsium,eval=FALSE>>=
...
\section{An example of analysis}
  ...
  The analysis presented here follow the very same steps described by
  ...
  In order to perform the analysis we must be sure that the analysis
  can be run, there are a few more fixes needed by our datasets.\\
  ...
  \item We change species names in the \texttt{traits} dataframe switching
    back to the short codes so that the same names used in
    \texttt{traits} and \texttt{veg\_data} are the same: 
@ 
<<a,eval=FALSE>>=
...
@ %def 
\item Then we need to perform and ordination on our \texttt{trait} dataframe; Kleyer et al.\cite{Kleyer2012} used a Principal component analysis (\texttt{dudi.pca} in \texttt{ade4}); in our case the \texttt{trait} dataframe contains both quantitative and factors variables, thus the \texttt{dudi.pca} can't be used; we therefor opt for \texttt{dudi.hillsmith} which accepts both type of variables.   
...
\item We can now plot an ordination graph of the species, grouped according to the newly defined functionale groups and plot, on top of that graph, traits data:
