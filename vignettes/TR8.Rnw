...
and then open that file with a spreadsheet software (e.g. LibreOffice).
  ...
  retrieved trait data will have all the found synonims a row names
  and one additional column $-$ named \textit{original\_names} $-$ will be
  added to the dataframe containing the results of the queries.
