...
\label{sec:usage}
  ...
  plant species names (\textbf{withouth authors' names!}\footnote{This is needed since some traitbases do not include authors' names in the species' names.})
...
\subsection{Interactive use of \texttt{tr8}}
  ...
  provide the users with very precise and detailed descriptions).
   ...
   Typically users will have a their vegetation data in the form of a
...
@ %def 
  ...
  \textbf{BEWARE}: this function takes a long time to run (nearly 20
  minutes are required - on a fast connection - for retrieving lookup
  tables for BiolFlor); it's very likely that the most recent version of \textsc{TR8} contains the already up-to-date version of the data, so in most cases will never need to use this function. 
